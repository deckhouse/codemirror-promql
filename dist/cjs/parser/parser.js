...
var Parser = /** @class */ (function () {
    ...
    Parser.prototype.checkAggregationExpr = function (node) {
        ...
        var body = node.getChild(lezer_promql_1.FunctionCallBody);
        var params = body ? body.getChildren('Expr') : [];
        if (!params.length) {
        ...
        if (aggregateOp.type.id === lezer_promql_1.CountValues) {
            this.expectType(params[0], types_1.ValueType.string, 'aggregation parameter');
        }
    };
    Parser.prototype.checkBinaryExpr = function (node) {
        ...
        var boolModifierUsed = node.getChild(lezer_promql_1.BoolModifier);
        var isComparisonOperator = (0, path_finder_1.containsAtLeastOneChild)(node, lezer_promql_1.Eql, lezer_promql_1.Neq, lezer_promql_1.Lte, lezer_promql_1.Lss, lezer_promql_1.Gte, lezer_promql_1.Gtr);
        var isSetOperator = (0, path_finder_1.containsAtLeastOneChild)(node, lezer_promql_1.And, lezer_promql_1.Or, lezer_promql_1.Unless);
        // BOOL modifier check
        if (boolModifierUsed) {
    ...
    Parser.prototype.checkCallFunction = function (node) {
        ...
        var body = node.getChild(lezer_promql_1.FunctionCallBody);
        var args = body ? body.getChildren('Expr') : [];
    ...
    Parser.prototype.checkVectorSelector = function (node) {
        var e_3, _a;
        var matchList = node.getChild(lezer_promql_1.LabelMatchers);
        ...
        try {
            for (var labelMatcherOpts_1 = __values(labelMatcherOpts), labelMatcherOpts_1_1 = labelMatcherOpts_1.next(); !labelMatcherOpts_1_1.done; labelMatcherOpts_1_1 = labelMatcherOpts_1.next()) {
                ...
                labelMatchers = labelMatchers.concat((0, matcher_1.buildLabelMatchers)(matchList ? matchList.getChildren(labelMatcherOpt) : [], this.state));
            }
        }
        ...
        var vectorSelectorNodeName = node.getChild(lezer_promql_1.Identifier);
        ...
        if (vectorSelectorName !== '') {
            ...
            var labelMatchersMetricName = labelMatchers.filter(function (lm) { return lm.name === '__name__'; });
            if (labelMatchersMetricName.length > 1) {
                this.addDiagnostic(node, "metric name must not be set twice: ".concat(labelMatchersMetricName[0].value, " or ").concat(labelMatchersMetricName[1].value));
            }
        }
